...
if __name__ == "__main__":
    work_dir = '/root/PLATFORM/pb_train/datasets/extinguisher/images/train'
    list_name = []
    listdir(work_dir, list_name)

    for i, path in enumerate(list_name):
        if i % 10 == 0:
            dst = '/root/PLATFORM/pb_train/datasets/extinguisher/images/val'
            os.system('mv {} {}'.format(path, dst))
            txt_path = path.replace('images','labels').replace('jpg','txt')
            if os.path.exists(txt_path):
                txt_dst = '/root/PLATFORM/pb_train/datasets/extinguisher/labels/val'
