...
if __name__ == "__main__":
    work_dir = '/root/PLATFORM/pb_train/datasets/mine_car/images/train'
    list_name = []
    listdir(work_dir, list_name)

    for i, path in enumerate(list_name):
        if i % 10 == 0:
            dst = '/root/PLATFORM/pb_train/datasets/mine_car/images/val'
            os.system('mv {} {}'.format(path, dst))
            txt_path = path.replace('images','labels').replace('jpg','txt')
            if os.path.exists(txt_path):
                txt_dst = '/root/PLATFORM/pb_train/datasets/mine_car/labels/val'
